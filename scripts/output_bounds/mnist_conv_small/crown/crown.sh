...
# Get the directory where the script is located (crown/)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Define project root (4 levels up from crown/)
...
# Check if crown.yaml exists
if [ ! -f "${CONFIG_DIR}/crown.yaml" ]; then
    echo "Error: ${CONFIG_DIR}/crown.yaml not found."
    exit 1
fi
